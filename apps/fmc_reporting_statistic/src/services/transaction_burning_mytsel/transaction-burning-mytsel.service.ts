...
@Injectable()
export class ReportTransactionBurningMyTselService {
  constructor(
  ...
  async reportingRedeemerMyTselUpdate(payload: any) {
    ...
    // update summary
    const redeemer = await this.countTrxBurningMyTselReporting(payload, '');
    const redeemer_indihome = await this.countTrxBurningMyTselReporting(
      payload,
      '_indihome',
    );
    const redeemer_telco = await this.countTrxBurningMyTselReporting(
      payload,
      '_telco',
    );
    ...
    await this.reportMonitoringModel
      .findOneAndUpdate(
        ...
        {
          trx_burn_mytelkomsel: redeemer,
          trx_burn_mytelkomsel_telco: redeemer_telco,
          trx_burn_mytelkomsel_indihome: redeemer_indihome,
        },
      )
      .then((res) => {
        return res;
  ...
  private async countTrxBurningMyTselReporting(payload: any, identifier: any) {
  ...
  private async getReportOlderDate(period, identifier: any) {
    ...
    try {
      console.log(`=>> TRX BURNING MYTSEL on ${period} query ...`);
      ...
      if (data) {
        if (identifier == '') {
          ress.year_to_date =
            data.trx_burn_mytelkomsel[
              data.trx_burn_mytelkomsel.length - 1
            ].year_to_date;
          ress.month_to_date =
            data.trx_burn_mytelkomsel[
              data.trx_burn_mytelkomsel.length - 1
          ...
          ress.year_to_date =
            data.trx_burn_mytelkomsel_indihome[
              data.trx_burn_mytelkomsel_indihome.length - 1
            ].year_to_date;
          ress.month_to_date =
            data.trx_burn_mytelkomsel_indihome[
              data.trx_burn_mytelkomsel_indihome.length - 1
          ...
          ress.year_to_date =
            data.trx_burn_mytelkomsel_telco[
              data.trx_burn_mytelkomsel_telco.length - 1
            ].year_to_date;
          ress.month_to_date =
            data.trx_burn_mytelkomsel_telco[
              data.trx_burn_mytelkomsel_telco.length - 1
      ...
      console.log(
        `=>> TRX BURNING MYTSEL on ${period} Exec time: ${Date.now() - start}`,
      );
    } catch (err) {
      // TODO
    }
  ...
  private async counterTransaction(payload: any, identifier: any) {
    ...
    console.log(
      `=>> Query Trx Burn MyTelkomsel: ${JSON.stringify(query_today)}`,
    );
    const count_today = await this.redeemModel.aggregate(query_today);

    console.log('startDate TRX BURN MyTelkomsel: ', start_period);
    console.log('endDate TRX BURN MyTelkomsel: ', end_period);
    ...
    console.log(
      `REPORTING TRX BURN MyTelkomsel  = ${
