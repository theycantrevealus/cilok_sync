...
export class ReportRedeemTransactionService {
      ...
      if (payload.parameter) {
        ...
        if (totalRecords > 0) {
          ...
          for (
            ...
            console.log(
              `pengolahan data report redeem transaction: ${batchEnd} of ${totalRecords}`,
            );
            for (let index = batchStart; index < batchEnd; index++) {
              const temporaryContent = [
                ...
                allowedIndihomeNumber(data[index].msisdn)
                  ? `${data[index].msisdn}`
                  : data[index].msisdn.substring(0, 1) === '8'
                  ? `62${data[index].msisdn}`
                  : data[index].msisdn,
                ...
                data[index].lacci,
                allowedIndihomeNumber(data[index].msisdn).toString(),
              ];
          ...
          // fs.appendFileSync(
          //   ctlDirFile,
          //   `${fileName}|${totalRecords}|${fileSizeInBytes}\n`,
          // );
  ...
  async createReportData(payload: any) {
    ...
    let customer_brand = '';
    if (Types.ObjectId.isValid(payload.customer.brand)) {
      customer_brand =
        payload.customer && payload.customer.brand
          ? await this.customerBrandModel
              .findById(payload.customer.brand)
              .then((result) => result.name)
          : null;
    } else {
      customer_brand =
        payload.customer && payload.customer.brand
          ? payload.customer.brand instanceof Array
            ? payload.customer.brand.join(',')
            : payload.customer.brand
          : null;
    }

    console.log(`Process customer brand data : ${customer_brand}`);
