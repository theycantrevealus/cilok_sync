...
export class ReportingFactDetailService {
  ...
  async addValue(payload): Promise<GlobalResponse> {
    ...
    if (payload.customer) {
      console.log(`Customer brand to check : ${payload.customer.brand}`);
      if (payload.customer.brand instanceof Array) {
        subscriberBrand = await this.customerBrandModel
          .findById(payload.customer.brand[0])
          .exec();
      } else {
        subscriberBrand = payload.customer.brand ?? '';
      }
    }
    ...
    console.log('Fact Detail Report Payload');
    console.log(factDetailPayload);
