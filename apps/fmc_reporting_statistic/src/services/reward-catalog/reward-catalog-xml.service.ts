...
export class RewardCatalogXMLService {
  ...
  private async getRewardCatalog(filter: any) {
    ...
    try {
      ...
      if (filter.keyword && filter.category && filter.merchant_id) {
        ...
        data = await this.keywordModel.aggregate(keywordPipeline);
      }
      return data;
    } catch (err) {
      console.error(err);
      ...
      if (data === undefined) {
        ...
        const rewardCatalog = data.map(async (i, index) => {
          try {
            ...
            // await this.serviceStock.deleteRedisStockSummary(i?._id);
            function isValidDateFormat(dateString) {
              // Check if the date string is valid (RFC2822/ISO format)
              return moment(dateString, moment.ISO_8601, true).isValid();
            }

            const effectiveDate = i?.reward_catalog?.effective;
            const toDate = i?.reward_catalog?.to;

            // Check if the provided dates are in a valid format before creating moment objects
            const startdateRC = isValidDateFormat(effectiveDate)
              ? moment(effectiveDate)
              : null;
            const enddateRC = isValidDateFormat(toDate) ? moment(toDate) : null;

            const isValidStart = startdateRC !== null && startdateRC.isValid();
            const isValidEnd = enddateRC !== null && enddateRC.isValid();
            // const dateNow = moment();
            // const effectiveTo = enddateRC;
            // const effectiveFrom = startdateRC;

            // const isEffectiveTo = enddateRC.isValid()
            //   ? effectiveTo.isBefore(dateNow)
            //   : true;
            // const isEffectiveFrom = startdateRC.isValid()
            //   ? effectiveFrom.isAfter(dateNow)
            //   : true;

            // const isStockAvailable =
            //   i?.eligibility?.flashsale?.status === true
            //     ? stockRemainingXML?.totalBalance !== 0 ||
            //       stockRemainingXML?.totalBalanceFlashsale !== 0
            //     : stockRemainingXML?.totalBalance !== 0;
            // // console.log(
            // //   '=== STOCK AVALAIBLE ===',
            // //   isStockAvailable,
            ...
            // Validate if the dates are valid before proceeding
            if (
              i.reward_catalog &&
              ...
              const startdateRC = moment(i.reward_catalog.effective);
              const enddateRC = moment(i.reward_catalog.to);
              const dateNow = moment();

              const isEffectiveTo = enddateRC.isBefore(dateNow);
              const isEffectiveFrom = startdateRC.isAfter(dateNow);
              ...
              if (isStockAvailable && !isEffectiveTo && !isEffectiveFrom) {
                let appliedOn = null;
                ...
                area_type = new Types.ObjectId(
                  await this.appsService.getConfig(
                    'DEFAULT_STATUS_LOCATION_AREA',
                  ),
                );

                region_type = new Types.ObjectId(
                  await this.appsService.getConfig(
                    'DEFAULT_STATUS_LOCATION_REGION',
                  ),
                );

                city_type = new Types.ObjectId(
                  await this.appsService.getConfig(
                    'DEFAULT_STATUS_LOCATION_CITY',
                  ),
                );

                // location_type = new mongoose.Types.ObjectId(
                //   i?.reward_catalog?.location_type,
                // );
                const bonus = i.bonus;
                if (bonus?.length > 0) {
                  for (const single_bonus of bonus) {
                    ...
                    switch (single_bonus?.bonus_type) {
                      ...
                      case 'e_auction':
                        try {
                          auction = await this.getEAuctionJSON(
                            single_bonus,
                            i?.eligibility?.program_id_info,
                          );
                        } catch (err) {
                ...
                if (i?.reward_catalog?.applied_on?.length > 0) {
                  if (i?.reward_catalog?.applied_on) {
                    ...
                    appliedOn?.forEach((applied) => {
                      ...
                      if (applied === 'is_indihome') {
                        appliedOnObj['IS_INDIHOME'] = 1;
                      }
                    });
                  }
                } else {
                ...
                specialAppliedOnObj['SPECIAL_INDIHOME'] = 0;
                if (i?.reward_catalog?.applied_on?.length > 0) {
                  if (i?.reward_catalog?.special_applied_on) {
                    ...
                    spesialappliedOn?.forEach((spclapplied) => {
                      ...
                      if (spclapplied === 'is_indihome') {
                        specialAppliedOnObj['SPECIAL_INDIHOME'] = 1;
                      }
                    });
                  }
                } else {
                ...
                if (i?.reward_catalog?.location_type == undefined && typeof i?.reward_catalog?.location_type === 'string') {
                  if (
                    i?.reward_catalog?.province ||
                    i?.reward_catalog?.city ||
                    i?.reward_catalog?.area ||
                    i?.reward_catalog?.region
                  ) {
                    if (Array.isArray(i?.reward_catalog?.city)) {
                      const cityUppercase = i?.reward_catalog?.city?.join(',');
                      cityObj = cityUppercase
                        ? cityUppercase?.toUpperCase()
                        : null;
                    } else if (
                    ...
                    if (Array.isArray(i?.reward_catalog?.province)) {
                      ...
                      typeof i?.reward_catalog?.province === 'string'
                    ) {
                      provinceObj = i?.reward_catalog?.province;
                    }

                    if (Array.isArray(i?.reward_catalog?.area)) {
                      const areaUppercase = i?.reward_catalog?.area?.join(',');
                      areaObj = areaUppercase
                        ? areaUppercase?.toUpperCase()
                        : null;
                    } else if (
                      Array.isArray(i?.reward_catalog?.area) &&
                      i?.reward_catalog?.area.length === 0
                    ) {
                      areaObj = 'NASIONAL';
                    } else if (typeof i?.reward_catalog?.area === 'string') {
                      areaObj = i?.reward_catalog?.area;
                    }

                    if (Array.isArray(i?.reward_catalog?.region)) {
                      const regionUppercase =
                        i?.reward_catalog?.region?.join(',');
                      regionObj = regionUppercase
                        ? regionUppercase?.toUpperCase()
                        : null;
                    } else if (
                      Array.isArray(i?.reward_catalog?.region) &&
                      i?.reward_catalog?.region.length === 0
                    ) {
                      regionObj = 'NASIONAL';
                    } else if (typeof i?.reward_catalog?.region === 'string') {
                      regionObj = i?.reward_catalog?.region;
                    }
                  }
                } else {
                  //LOCATION TYPE AREA
                  const location_type_id = new mongoose.Types.ObjectId(i?.reward_catalog?.location_type?._id)
                  console.log('=== CHECK ===', location_type_id);
                  console.log('=== CHECK TYPE ===', area_type);

                  //Lempar detail type region,prov,city
                  if (area_type.equals(location_type_id)) {
                    if (Array.isArray(i?.reward_catalog?.area)) {
                      const areaUppercase = i?.reward_catalog?.area?.join(',');
                      areaObj = areaUppercase
                        ? areaUppercase?.toUpperCase()
                    ...
                    const regionDetails =
                      await this.locationService.getLocationDetailRedis(
                        i?.reward_catalog?.area.map((area) => ({
                          name: area,
                          detail_type: 'region',
                          typeId: area_type,
                        })),
                      );
                    regionObj = regionDetails
                    ...
                    cityObj = cityDetails
                      .map((detail) => detail.result)
                      .join(', ');
                  } else if (region_type.equals(location_type_id)) {
                    if (Array.isArray(i?.reward_catalog?.area)) {
                      const areaUppercase = i?.reward_catalog?.area?.join(',');
                      areaObj = areaUppercase
                        ? areaUppercase?.toUpperCase()
                    ...
                    const provinceDetails =
                      await this.locationService.getLocationDetailRedis(
                        i?.reward_catalog?.region.map((region) => ({
                          name: region,
                          detail_type: 'prov',
                          typeId: region_type,
                        })),
                    ...
                    cityObj = cityDetails
                      .map((detail) => detail.result)
                      .join(', ');
                  } else if (city_type.equals(location_type_id)) {
                    if (Array.isArray(i?.reward_catalog?.area)) {
                      const areaUppercase = i?.reward_catalog?.area?.join(',');
                      areaObj = areaUppercase
                        ? areaUppercase?.toUpperCase()
                    ...
                    if (Array.isArray(i?.reward_catalog?.city)) {
                      ...
                      areaObj = i?.reward_catalog?.city;
                    }
                  } else {
                    if (typeof i?.reward_catalog?.area === 'string') {
                      areaObj = i?.reward_catalog?.area;
                    } else {
                      areaObj = 'NASIONAL';
                    }
                    if (typeof i?.reward_catalog?.region === 'string') {
                      regionObj = i?.reward_catalog?.region;
                    } else {
                      regionObj = 'NASIONAL';
                    }
                    if (typeof i?.reward_catalog?.province === 'string') {
                      provinceObj = i?.reward_catalog?.province;
                    } else {
                      provinceObj = 'NASIONAL';
                    }
                    if (typeof i?.reward_catalog?.city === 'string') {
                      cityObj = i?.reward_catalog?.city;
                    } else {
                      cityObj = 'NASIONAL';
                    }
                ...
                if (benefitCategory && typeof benefitCategory === 'string') {
                  ...
                  if (objectId) {
                    ...
                    if (
                      foundDocument &&
                      foundDocument.set_value !== undefined
                    ) {
                ...
                //JIKA START AND END DATE UDAH MASUK HITUNGAN FLASHSALE TAG XML FLASH SALE KEBENTUK
                // let effectiveToFlashsale;
                // let effectiveFromFlashsale;
                // let isEffectiveToFlashsale;
                // let isEffectiveFromFlashsale
                // if(i?.reward_catalog?.flashsale?.status){
                //   const dateNowFS = moment();
                //   effectiveToFlashsale = moment(i?.reward_catalog?.flashsale?.start_date);
                //   effectiveFromFlashsale = moment(i?.reward_catalog?.flashsale?.end_date);

                //   // Logika diperbaiki untuk menghasilkan false ketika dateNowFS berada di dalam range start_date dan end_date
                //   isEffectiveToFlashsale = dateNowFS.isBefore(effectiveToFlashsale) || dateNowFS.isAfter(effectiveFromFlashsale);
                //   isEffectiveFromFlashsale = dateNowFS.isBefore(effectiveToFlashsale) || dateNowFS.isAfter(effectiveFromFlashsale);

                //   console.log('=== FS ===',i?.reward_catalog?.keyword)
                //   console.log('=== FS TO ===',isEffectiveToFlashsale)
                //   console.log('=== FS FROM ===',isEffectiveFromFlashsale)
                //   console.log('=== FS NOW ===',dateNowFS)
                ...
                const ROW = {
                  ...
                  EVENT_PERIOD:
                    startPeriod.isValid() && endPeriod.isValid()
                      ? `${startPeriod.format(
                          'DD MMM YYYY',
                        )} sd ${endPeriod.format('DD MMM YYYY')}`
                      : '',
                  EXPIRY_PERIOD: endPeriod.isValid()
                  ...
                  FLASHSALE:
                    i?.eligibility?.flashsale?.status == true
                      ? await this.getFlashSale(
                          i?.eligibility?.flashsale,
                          i?.reward_catalog?.flashsale,
                          stockRemainingXML?.totalBalanceFlashsale,
                        )
                      : '',
                };
                ...
                if (stockRemainingXML == null) {
                  ...
                  ROW.STOCK = stockRemainingXML?.totalBalance;
                }
  ...
  // async generateRewardCatalogXMLOld(
  //   filter: RewardCatalogFilterDTO,
  //   payload: any,
  // ): Promise<any> {
  //   const serviceResult = new ReportingServiceResult({
  ...
  async getEAuctionJSON(payload, keyword_registration): Promise<any> {
    let image_auction = null;

    if (payload.auction_prize_image) {
      const imageAuctionParts = payload.auction_prize_image?.split('/img');
      image_auction = imageAuctionParts ? 'img' + imageAuctionParts[1] : '';
    } else {
      image_auction = 'img/image_merchant/detail/';
    }

    const e_auction_json: any = {
      PRIZE_IMAGE: image_auction || '',
      ...
      KEYWORD_REGISTRATION: keyword_registration?.keyword_registration || '',
      MINIMUM_BID_POIN: payload.auction_poin_min_bidding || '',
    };

    if (payload?.auction_multiplier_identifier === 'LAST_BID') {
      e_auction_json.MULTIPLIER_POIN = {
        LAST_BID: 'TRUE',
        POIN_VALUE: '',
      };
    } else if (payload?.auction_multiplier_poin) {
      e_auction_json.MULTIPLIER_POIN = {
        LAST_BID: 'FALSE',
        POIN_VALUE: payload.auction_multiplier_poin,
      };
    } else {
      e_auction_json.MULTIPLIER_POIN = '';
    }

    return e_auction_json;
  }
  ...
  async getStockRemaining(payload: any): Promise<any> {
    try {
      ...
      // if (stockRemaining && stockBalance) {
      //   // console.log('== ID KEY ==', stockRemaining[0]?._id);
      //   await this.stockSummaryModel.findOneAndUpdate(
      //     {
      //       product: product_idMain,
      //       keyword: new mongoose.Types.ObjectId(stockRemaining[0]?._id),
      //     },
      //     {
      //       balance: stockBalance?.totalBalance,
      //     },
      //     {
      //       upsert: true,
      //     },
      //   );
      // }
      return stockBalance ? stockBalance : null;
    } catch (error) {
      return null;
    }
  }
  ...
  async getFlashSale(
    payload_eligi,
    payload_rc,
    stockFlashsaleRemaining,
  ): Promise<any> {
    let image_flashsale = null;

    if (payload_rc.flashsale_image) {
      const imageFlashsaleParts = payload_rc.flashsale_image?.split('/img');
      image_flashsale = imageFlashsaleParts
        ? 'img' + imageFlashsaleParts[1]
        : '';
    } else {
      image_flashsale = 'img/image_merchant/detail/';
    }

    const flashsale_json: any = {
      START_DATE:
        moment(payload_eligi?.start_date).format('YYYY-MM-DDTHH:mm:ss.SSSZ') ||
        '',
      END_DATE:
        moment(payload_eligi?.end_date).format('YYYY-MM-DDTHH:mm:ss.SSSZ') ||
        '',
      POIN: payload_eligi.poin || '',
      STOCK: stockFlashsaleRemaining,
      IMAGE: image_flashsale || '',
      DESCRIPTION: payload_rc?.flashsale_description || '',
    };

    return flashsale_json;
  }
}
