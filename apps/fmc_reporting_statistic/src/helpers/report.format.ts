...
const getTableHeader = (index, reportName, data) => {
  ...
  html.push(`<thead>`);
  const dataKey = [];
  if (!['reward_live_system','program','reward_trx'].includes(reportName)) dataKey.push('period');
...
const getTableData = (index, reportName, data) => {
  const html = [];
  const dataKey = [];
...
const formatTable = (payload) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (data.length > 1) {
      ...
      const tbody = getTableData(i, reportName, data);
    ...
    // CUSTOM
    if (i == 6 || i == 11 || i == 16 || i == 21) html.push(`</table></tbody><table nowrap> <tbody>`);
