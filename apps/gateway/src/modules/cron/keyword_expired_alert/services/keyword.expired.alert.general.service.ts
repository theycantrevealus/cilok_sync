...
import { KeywordExpiredAlertGeneralJobService } from './keyword.expired.alert.general.job.service';

@Injectable()
export class KeywordExpiredAlertGeneralService {

  constructor(
    private schedulerRegistry: SchedulerRegistry,
    @InjectModel(Lov.name) private lovModel: Model<LovDocument>,
    private keywordExpiredAlertGeneralJobService: KeywordExpiredAlertGeneralJobService,
  ) { }
  ...
  cronSetup() {
    ...
    const data = {
      "group_name": 'CRONJOB',
      "set_value": 'KEYWORD_EXPIRED_ALERT_GENERAL', //unique name of cronjob
  ...
  @Cron('* * * * * *', {
    name: 'KEYWORD_EXPIRED_ALERT_GENERAL' //unique name of cronjob (must be same with cronSetup value)
  })
  async handleCron() {
  ...
  addCronJob(name: string, cronTime: string, cronSetup:any) {
    const job = new CronJob(cronTime, async () => {
      this.keywordExpiredAlertGeneralJobService.runTheJobs(cronSetup);
    });
