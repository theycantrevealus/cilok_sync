...
import { KeywordQuotaAlertJobService } from './keyword.quota.alert.job.service';

@Injectable()
export class KeywordQuotaAlertService {

  constructor(
    private schedulerRegistry: SchedulerRegistry,
    @InjectModel(Lov.name) private lovModel: Model<LovDocument>,
    private keywordExpiredAlertSpecificJobService: KeywordQuotaAlertJobService,
  ) { }
  ...
  cronSetup() {
    ...
    const data = {
      "group_name": 'CRONJOB',
      "set_value": 'KEYWORD_QUOTA_ALERT', //unique name of cronjob
      "description": 'Send email notification while keyword quota is < threshold',
      "additional": {
        'cronjob': {
  ...
  @Cron('* * * * * *', {
    name: 'KEYWORD_QUOTA_ALERT' //unique name of cronjob (must be same with cronSetup value)
  })
  async handleCron() {
