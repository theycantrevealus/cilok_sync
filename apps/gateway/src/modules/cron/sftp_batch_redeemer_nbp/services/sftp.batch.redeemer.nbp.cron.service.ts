...
import { SftpbatchRedeemerNbpJobService } from './sftp.batch.redeemer.nbp.job.service';

@Injectable()
export class SftpbatchRedeemerNbpCronService {

  constructor(
    private schedulerRegistry: SchedulerRegistry,
    @InjectModel(Lov.name) private lovModel: Model<LovDocument>,
    private jobService: SftpbatchRedeemerNbpJobService,
  ...
  cronSetup() {
    ...
    const data = {
      "group_name": 'CRONJOB',
      "set_value": 'SFTP_BATCH_REDEEMER_NBP', //unique name of cronjob
      "description": 'SFTP for generate customer redeemer data by specific keyword (Keyword “0Poin” & “5Poin” ) and send file to another server',
      "additional": {
        'cronjob': {
        ...
        'data': {
          ...
          generate_dir: './generate/keyword_redeemer_nbp',
          server_destination: [
            {
              host: '18.143.54.48',
              port: 22,
  ...
  @Cron('* * * * * *', {
    name: 'SFTP_BATCH_REDEEMER_NBP' //unique name of cronjob (must be same with cronSetup value)
  })
  async handleCron() {
