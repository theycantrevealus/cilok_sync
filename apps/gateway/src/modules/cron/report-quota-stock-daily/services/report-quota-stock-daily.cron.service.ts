...
import { ReportQuotaStockDailyJobService } from './report-quota-stock-daily.job.service';

const CRONJOB_NAME = 'REPORT_QUOTA_STOCK_DAILY';
const CRONJOB_DESCRIPTION ='Daily cronjob for Report Stock';

@Injectable()
export class ReportQuotaStockDailyCronService {
  constructor(
    private schedulerRegistry: SchedulerRegistry,
    @InjectModel(Lov.name) private lovModel: Model<LovDocument>,
    private reportQuotaStockDailyJobService: ReportQuotaStockDailyJobService,
  ) {}
  ...
  addCronJob(name: string, cronTime: string, data: CronDataDTO) {
    const job = new CronJob(cronTime, async () => {
      this.reportQuotaStockDailyJobService.runTheJobs(name.replace('_SUB', ''), data);
    });
