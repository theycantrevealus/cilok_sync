...
export class InjectProcessor {
  ...
  constructor(
    ...
    @InjectModel(InjectCouponModel.name)
    private listModelCoupon: Model<InjectCouponModelDocument>,

    @InjectModel(InjectPointModel.name)
    private listModelPoint: Model<InjectPointModelDocument>,

    @InjectModel(RedeemModel.name)
    private listModelRedeem: Model<RedeemModelDocument>,
  ) {
  ...
  @Process('inject-coupon')
  async handleInjectCoupon(job: Job): Promise<void> {
                                ...
                                $and: [
                                  {
                                    ...
                                    keyword: doc.keyword,
                                    total_coupon: parseInt(doc.total_coupon),
  ...
  @Process('redeem')
  async handeRedeemModel(job: Job): Promise<void> {
    if (job.data.list.filename) {
      return new Promise((resolve, reject) => {
        let countTotalLine = 0;
        fs.createReadStream(job.data.list.path)
            ...
            try {
              ...
              for await (const line of rl) {
                ...
                bulkData.push({
                  locale: 'en-US',
                  msisdn: dataSplit[0],
                  keyword_redeem: job.data.parameter.keyword_redeem,
                  total_redeem: dataSplit[1],
                });
                                ...
                                $and: [
                                  {
                                    locale: doc.locale,
                                    msisdn: doc.msisdn,
                                    keyword_redeem: doc.keyword,
                                    total_redeem: doc.total_redeem,
                                  },
                                ],
                              },
                              update: doc,
