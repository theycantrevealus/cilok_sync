...
export class PrepaidGranularController {
  ...
  // prepaid granular
  @MessagePattern(process.env.KAFKA_PREPAID_GRANULAR_RENEWAL_TOPIC)
  ...
  async decodeSR(message: KafkaMessage) {
    const registry = new SchemaRegistry({
      host: this.configService.get<string>(
        'kafka.prepaid_granular_renewal.schema_registry.host',
      ),
      auth: {
        username: this.configService.get<string>(
          'kafka.prepaid_granular_renewal.schema_registry.username',
        ),
        password: this.configService.get<string>(
          'kafka.prepaid_granular_renewal.schema_registry.password',
        ),
      },
