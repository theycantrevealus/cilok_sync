...
@Processor('inject')
export class InjectCouponProcessor {
  ...
  async handleInjectCouponModel(job: Job): Promise<void> {
        ...
        fs.createReadStream(job.data.list.path)
          ...
          .on('end', async () => {
            ...
            try {
              ...
              rl.on('close', async () => {
                const oldPath = job.data.list.path;
                const newPath = `${job.data.list.path}.gz`;
