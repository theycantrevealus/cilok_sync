...
@Injectable()
export class InjectPointService {
  private inject_point_dir: string;
  ...
  constructor(
    ...
    this.inject_point_dir = `${configService.get<string>('inject_point.dir')}`;
  }
  ...
  async process(payload: any, ip: string): Promise<any> {
    ...
    fs.readdir(this.inject_point_dir, async (err, files) => {
