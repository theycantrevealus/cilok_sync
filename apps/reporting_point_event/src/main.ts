...
import { ReportingPointEventModule } from './reporting_point_event.module';
...
async function bootstrap() {
  ...
  const app = await NestFactory.createMicroservice<MicroserviceOptions>(
    ReportingPointEventModule,
    await KafkaConn.reporting_point_event[0].useFactory(configService),
  );
