...
export class ReportingGenerationService {
  constructor(
    ...
    @Inject(CampaignServiceV1)
    private readonly campaignService: CampaignServiceV1,

    @Inject(PointEarningService)
  ...
  async reportGenerationRouting(payload) {
    ...
    const isFMC = payload?.parameter?.is_fmc ?? false;
    ...
    try {
      ...
      if (payload?.origin?.toString()?.toUpperCase() == 'CRON') {
        ...
        /**
         * RESET FLASHSALE KEYWORD STOCK
         */
        if (
          payload?.service_name?.toUpperCase() ==
          'RESET_FLASHSALE_KEYWORD_STOCK'
        ) {
          const delayTime = payload?.parameter?.delay_time;

          const resultResetFlashSaleKeywordStock =
            await this.stockService.cronResetStockFlashSale(delayTime);
          console.log('result RESET_FLASHSALE_KEYWORD_STOCK : ');
          console.log(resultResetFlashSaleKeywordStock);
          await this.checkAndSetLog(
            'RESET FLASH SALE KEYWORD STOCK - Reporting RESET FLASH SALE KEYWORD STOCK',
            resultResetFlashSaleKeywordStock,
            payload,
            startTime,
          );
        }
         ...
         * BROADCAST CAMPAIGN
         */
        if (payload?.service_name?.toUpperCase() == 'BROADCAST_CAMPAIGN') {
          const broadcastCampaign =
            await this.campaignService.monitorBroadcast();
          console.log('result BROADCAST_CAMPAIGN : ');
          console.log(broadcastCampaign);
          await this.checkAndSetLog(
            'BROADCAST_CAMPAIGN',
        ...
        if (
          payload?.service_name?.toUpperCase() ==
          'REWARD_CATALOG_GENERATE_LOCATION_DETAIL'
        ) {
          this.locationService.generateLocationDetail(payload);
        ...
        if (
          payload?.service_name.toUpperCase() == 'BULK_UPDATE_EXPIRED_VOUCHER'
        ) {
          const response: ReportingServiceResult =
  ...
  async reportGenerationRecoveryRouting(payload) {
    ...
    try {
      ...
      for (let i = 0; i < dateRanges.length; i++) {
        ...
        if (payload?.service_name?.includes('EARNING_POINT')) {
          const resultEarningProcess = await this.pointEarningService.cronEarningRetry();
          // check and set log
          await this.checkAndSetLog(
            'EARNING_POINT_CRON',
            resultEarningProcess,
            payload,
            startTime,
          );
        }
