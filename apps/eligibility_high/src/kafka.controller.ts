...
export class KafkaController {
  ...
  @MessagePattern(process.env.KAFKA_ELIGIBILITY_HIGH_TOPIC)
      ...
      await this.kafkaService
        ...
        .then(async (response) => {
          ...
          if (
            ...
            // await this.transactionRecoveryService.setStep(payload, process.env.KAFKA_NOTIFICATION_TOPIC);
          } else if (last_origin === 'eligibility_success') {
            if (response?.notification) {
              console.log('notif agung', response?.notification);
            }
            return await this.deductClient.emit(
              process.env.KAFKA_DEDUCT_HIGH_TOPIC,
              response,
            );
          ...
          if (last_origin !== 'eligibility_fail') {
            // await this.transactionRecoveryService.setStep(payload, process.env.KAFKA_DEDUCT_HIGH_TOPIC);
          }
