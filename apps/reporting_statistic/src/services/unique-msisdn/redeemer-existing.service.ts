...
export class ReportingRedeemerExistingService {
  ...
  private async getReportOlderDate(period) {
    ...
    try {
      ...
      console.log('DATANYA', data?.redeemer_existing);

      if (data) {
        ress.year_to_date = data.redeemer_existing[2].year_to_date; // today
      }
  ...
  async countDateToday(payload: any) {
    if (payload.historyData) {
      ...
      let year_to_date = 0;
      year_to_date = data.redeemer_existing[2].year_to_date; // today
      const result = {
        name: 'Date Today',
        period: moment(payload.period).format('YYYY-MM-DD'),
