...
export class ReportTransactionBurningMyTselService {
  ...
  private async countTrxBurningMyTselReporting(payload: any) {
  ...
  private async getReportOlderDate(period) {
    ...
    try {
      ...
      console.log('DATANYA', data?.trx_burn_mytelkomsel);

      if (data) {
        ress.year_to_date = data.trx_burn_mytelkomsel[2].year_to_date; // today
        ress.month_to_date = data.trx_burn_mytelkomsel[2].month_to_date; // today
      }
