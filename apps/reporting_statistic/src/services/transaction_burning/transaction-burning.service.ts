...
export class ReportTransactionBurningService {
  ...
  private async countTrxBurningReporting(payload: any) {
  ...
  private async getReportOlderDate(period) {
    ...
    try {
      ...
      console.log('DATANYA', data?.trx_burn);

      if (data) {
        ress.year_to_date = data.trx_burn[2].year_to_date; // today
        ress.month_to_date = data.trx_burn[2].month_to_date; // today
      }
