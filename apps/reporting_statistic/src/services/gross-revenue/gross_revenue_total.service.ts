...
export class GrossRevenueTotalService {
  ...
  private async countDateToday(period) {
    ...
    year_to_date = data.gross_revenue[2].year_to_date; // today
    month_to_date = data.gross_revenue[2].month_to_date; // today
  ...
  private async getReportOlderDate(period) {
    ...
    try {
      ...
      console.log('DATANYA', data?.gross_revenue);

      if (data) {
        ress.year_to_date = data.gross_revenue[2].year_to_date; // today
        ress.month_to_date = data.gross_revenue[2].month_to_date; // today
      }
