  ...
  async reportMonitoringRouting(@Payload() payload: any) {
    ...
    try {
      ...
      if (payload?.transaction_classify == 'REDEEM') {
        if (payload?.transaction_status?.toUpperCase() == 'SUCCESS') {
          ...
          */
          ...
          await this.reportRedeemTransactionService.createReportData(payload);
          // logger
          await this.loggerReportStatistic(
            loggerObject,
            false,
        ...
        // }
          ...
          console.error('Unknown transaction_status value!');
          await this.loggerReportStatistic(
            loggerObject,
            false,
